...
async function botNameApprove(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    "Якщо в тебе залишилися запитання, чи буде потрібна допомога у вирішенні конфлікту шляхом медіації - напиші моїм Земним друзям в Українську академію медіації (УАМ). ✨✨✨ Вони володіють магією медіації та обов’язково тобі допоможуть. УАМ також має сайт з купою корисної інформації 🙃",
    {
      reply_markup: JSON.stringify({
        inline_keyboard: [
          [
