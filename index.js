...
async function startScreen(curUser) {
  if (!curUser.started) {
    ...
    await bot.sendMessage(
      curUser.telegramId,
      `Раді вітати у грі-квизі «ЧАРІВНИЙ СВІТ МЕДІАЦІЇ»!
      \nПропонуємо перевірити, що ти знаєш про конфлікти, як їх вирішувати і як себе в них поводити. Також запрошуємо тебе доторкнутися до чарівного світу медіації за допомогою медіатора-мандрівника, який прибув з дружньої планети. Він буде давати тобі завдання та ставити запитання. Будь уважним, адже на кожне питання є декілька відповідей і лише одна з них буде правильною. Правильні відповіді на запитання будуть магічно перетворюватися на бали. Також в тебе є можливість отримати бали за виконання додаткових творчих завдань.
      \nТільки не засмучуйся, якщо правильна відповідь не підкориться тобі з першого разу – це можливість дізнатися нову та корисну інформацію. Пробуй різні варіанти, читай пояснення та знаходь вірну відповідь як справжній дослідник. У тебе завжди є можливість дізнатися, чому інші відповіді не є правильними - для цього просто на них натисни. 
      ...
      \nКожного місяця визначатимуться п’ять переможців, які першими наберуть максимальну кількість балів. 
...
async function nameStory(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    "Знаєш, якщо ти хочеш мати приємне спілкування з іншою людиною, важливо знати його ім’я та звертатися на ім'я. Один мудрець сказав, що ім’я — найсолодший і найважливіший для людини звук будь-якою мовою"
  );
  await bot.sendMessageDelay(
    curUser,
...
async function nameStory2(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    "Я теж є медіатором з надзвичайно важливою місією: мандрую різними планетами і світами та ділюся досвідом, знаннями і навичками 😁"
  );
  await bot.sendMessageDelay(
    curUser,
...
async function checkMediatorAnswer(curUser, text) {
  if (text === "1") {
    ...
    await bot.sendMessageDelay(
      curUser,
      "Правильно! Є медіатор, який використовується для гри на гітарі, але ми будемо спілкуватися про Медіаторів, які проводить медіацію",
      { reply_markup: JSON.stringify({ hide_keyboard: true }) }
    );
    curUser.isMediatorAnswerWriting = false;
    await startQuizQuestion(curUser);
...
async function afterQuiz(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    `Я вмію уважно слухати, наче маю чарівні вуха, щоб порозумітися`
  );
  await bot.sendMessageDelay(
    curUser,
    `Я майстерно розмовляю і при цьому вмію тримати язик за зубами. Не розповідаю іншим про те, що дізнався від сторін конфлікту`
  );
  await bot.sendMessageDelay(
    curUser,
  ...
  await bot.sendMessageDelay(
    curUser,
    `Я однаково доброзичливо ставлюсь до всіх сторін конфлікту, не оцінюю та не засуджую їх`
  );
  await bot.sendMessageDelay(
    curUser,
...
async function botNameApprove(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    `Вітаю! Тобі вдалося успішно пройти цей квест та набрати так багато балів! Цілих ${curUser.points} 😱`
  );
  await bot.sendMessageDelay(
    curUser,
    `В останній день цього місяця ми дізнаємося, кому вдалося першим набрати максимальну кількість балів в цьому місяці. Вони отримають приз відповідно до Правил (сделать активной ссілкой)`
  );
  await bot.sendMessageDelay(
    curUser,
  ...
  await bot.sendMessageDelay(
    curUser,
    "Якщо в тебе залишилися питання чи просто хочеш дізнатися більше про медіацію, то напиши мені",
    {
      reply_markup: JSON.stringify({
        inline_keyboard: [
          [
