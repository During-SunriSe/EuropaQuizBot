...
async function sendInfo(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    "•	Друге завдання - я ставлю питання, а ти на них відповідаєш. За кожну вірну відповідь тобі нараховуються бали. На кожне питання є декілька відповідей. Будь уважним, не всі з них є вірними. Чим більше правильних відповідей з першої спроби, тим більше балів. Ти не обмежений в часі і кількості спроб."
  );
  await bot.sendMessageDelay(
    curUser,
...
async function nameStory2(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    "На нашій планеті ні сварок, ні бійок тому, що всіх змалку навчають, що конфлікти це не погано та іноді навіть добре. Важливо, якщо вони виникають, то вирішувати їх треба шляхом перемовин"
  );
  await bot.sendMessageDelay(
    curUser,
