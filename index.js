...
async function startScreen(curUser) {
  if (!curUser.started) {
    ...
    await bot.sendMessage(
      ...
      \nТільки не засмучуйся, якщо правильна відповідь не підкориться тобі з першого разу – це можливість дізнатися нову та корисну інформацію. Пробуй різні варіанти, читай пояснення та знаходь вірну відповідь як справжній дослідник. 🕵🕵‍♀️
      ...
      \nКожного місяця визначатимуться 10 переможців, які першими наберуть максимальну кількість балів. 
...
async function botNameApprove(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    `В останній день цього місяця ми дізнаємося, кому вдалося першим набрати максимальну кількість балів в цьому місяці. Вони отримають призи відповідно до Правил (сделать активной ссілкой)`
  );
  await bot.sendMessageDelay(
    curUser,
