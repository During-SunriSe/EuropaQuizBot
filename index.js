...
async function startScreen(curUser) {
  if (!curUser.started) {
    ...
    await bot.sendMessage(
      curUser.telegramId,
      `Раді вітати у грі-кесті «ЧАРІВНИЙ СВІТ МЕДІАЦІЇ»!
      ...
      \nТільки не засмучуйся, якщо правильна відповідь не підкориться тобі з першого разу – ти отримаєш трохи менше балів але дізнаєшься нову та корисну інформацію. Пробуй різні варіанти, читай пояснення та знаходь вірну відповідь як справжній дослідник. 🕵🕵‍♀️
      ...
      \nКожного місяця визначатимуться 30 переможців, які першими наберуть максимальну кількість балів. 
...
async function botNameApprove(curUser) {
  ...
  await bot.sendMessageDelay(
    curUser,
    `В останній день цього місяця ми дізнаємося, кому вдалося першим набрати максимальну кількість балів в цьому місяці. Вони отримають призи відповідно до <a href='https://docs.google.com/document/d/1Kw3fMgjkdaYNg2-k4cYk6S6gjBu1RkfF3N1qGmQ0xHg/edit?usp=sharing'>правил</a>`,
    { parse_mode: "HTML" }
  );
  await bot.sendMessageDelay(
    curUser,
